...
describe("wireWorkspaceGenericPack sandbox (EDR-014)", () => {
  it("injects rejecting sandbox and never allowlists sandbox.exec by default", async () => {
  ...
  it("opt-in registers sandbox.exec and can run allowlisted node", async () => {
    ...
    expect(pack.toolAllowlist).toContain("sandbox.exec");
    expect(pack.registry.getToolAllowlist()).toContain("sandbox.exec");
