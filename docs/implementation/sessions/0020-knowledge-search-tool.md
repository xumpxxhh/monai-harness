# Session 0020 — RAG `knowledge_search` Tool（EDR-016）
...
按 [docs/rag/agent-integration.md](../../rag/agent-integration.md) 接入自研 RAG HTTP 为 Pack Tool **`knowledge_search`**（模型按需调用），**不**实现 KnowledgePort 预检索。
...
3. Goal 含事实问题；模型应调用 `knowledge_search`；hits 进入 tool 消息与 recent facts
