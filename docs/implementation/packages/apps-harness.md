...
| --- | --- |
| 计划路径 | `apps/harness/` |
| 状态 | `in_progress`（**P8b bootstrap 完成**；P8c：HTTP） |
| 首触阶段 | P0（空壳）起贯穿 |
...
| 最后更新 | 2026-08-27（P8b：DI + delivery + PG demo） |
...
- [ ] HTTP server 启动 — P8c（路由见 [api.md](./api.md)）
- [ ] 角色可独立开关（便于测试）
...
几乎所有 packages/adapters（运行期）。已链接 contracts/ports/runtime/api/delivery/observability/pack-sdk/model-stub/persistence-memory/**persistence-postgres**/queue-memory/synthetic-sink。

## 5. 缺口与风险

- HTTP/SSE 仍 Deferred（EDR-007）— P8c
- 角色独立开关未做；serve 模式尚无 HTTP
